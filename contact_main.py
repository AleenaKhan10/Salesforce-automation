...
def run(batch_size: int = 100, sf: Salesforce | None = None) -> dict:
    """Run one pass of the contact job and return summary stats."""
    if sf is None:
        sf = connect()
        log.info("Connected to Salesforce (contact-only)")
    else:
        log.info("Using provided Salesforce session (contact-only)")
    ...
    return {
        "success": True,
        "processed": len(dms),
        "flushed_contact_methods": flushed,
        "totals": totals,
    }


# === FastAPI entry point ===
def run_contacts_job_once(sf: Salesforce, batch_size: int = 500) -> dict:
    """Entry point for FastAPI orchestrator: run contacts job once."""
    try:
        return run(batch_size=batch_size, sf=sf)
    except Exception as e:
        log.exception("run_contacts_job_once failed: %s", e)
        return {"success": False, "error": str(e)}



if __name__ == "__main__":
    # One-shot execution only (no looping)
    bs = 500
