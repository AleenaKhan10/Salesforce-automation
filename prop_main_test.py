...
def run_properties_job_once(sf: Salesforce, batch_size: int | None = None) -> dict:
    """FastAPI entry point: run the property job exactly once using an existing Salesforce session."""
    try:
        # Allow API to override batch size without changing env
        ...
        logger.exception("run_properties_job_once failed: %s", e)
...
if __name__ == "__main__":
    # Always run ONE pass when executed directly.
    # (FastAPI should import this module and call run_properties_job_once(sf) instead.)
    main()
